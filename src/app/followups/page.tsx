...
export default async function FollowupsPage() {
  const [followup1Items, followup2Items] = await Promise.all([
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--ink-soft)]">
          Today's pending follow-ups, each sent only from the same account that sent email 1.
        </p>
      </div>

      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[var(--ink)]">2nd Follow-up</h2>
        <TodaySendingView bucket="followup1" initialItems={followup1Items} />
      </section>

      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[var(--ink)]">3rd Follow-up</h2>
        <TodaySendingView bucket="followup2" initialItems={followup2Items} />
      </section>
    </div>
  );
}
